...
* 版本: 0.6 -> 2026-03-27：添加两个打印级别 trace、success
    SHA256：CF1596F2F9AF0C0FEB044D494FCAD9350949DF00516D92FF82E13920029477BC
* 备注：
* 示例:
...
output_group.add_argument('--level', type=str, default='trace', choices=['trace', 'debug', 'info', 'success', 'warning', 'error', 'critical'], help='显示日志等级')
    ...
    if request.json["level"] == "DEBUG":
        ...
        try:
            ...
            logger.critical("", cusfile=jsondata['file'], cusfunction=jsondata['function'],
                         ...
                         cusprocess=jsondata['process'], mname=jsondata['mname'])
            print(jsondata['message'])
    elif request.json["level"] == "TRACE":
        try:
            logger.trace(str(jsondata['message']), cusfile=jsondata['file'], cusfunction=jsondata['function'],
                            cusline=jsondata['line'], cusname=jsondata['process'],
                            custhread=jsondata['thread'], cusprocess=jsondata['process'], mname=jsondata['mname'])
        except:
            logger.trace("", cusfile=jsondata['file'], cusfunction=jsondata['function'],
                            cusline=jsondata['line'], cusname=jsondata['process'], custhread=jsondata['thread'],
                            cusprocess=jsondata['process'], mname=jsondata['mname'])
            print(jsondata['message'])
    elif request.json["level"] == "SUCCESS":
        try:
            logger.success(str(jsondata['message']), cusfile=jsondata['file'], cusfunction=jsondata['function'],
                            cusline=jsondata['line'], cusname=jsondata['process'],
                            custhread=jsondata['thread'], cusprocess=jsondata['process'], mname=jsondata['mname'])
        except:
            logger.success("", cusfile=jsondata['file'], cusfunction=jsondata['function'],
                            cusline=jsondata['line'], cusname=jsondata['process'], custhread=jsondata['thread'],
                            cusprocess=jsondata['process'], mname=jsondata['mname'])
            print(jsondata['message'])
